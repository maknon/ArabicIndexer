		...
		final ActionListener SearchActionListener = new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent e)
			{
				...
				else
				{
					if (!searchThreadWork)
					{
						// Version 2.1, avoid 'IndexSearcher$TooManyClauses: maxClauseCount is set to 1024'
								...
								try
								{
									...
									if (/*!filterQuery.isEmpty()*/!booleanQuery.toString().isEmpty() || (fullSearch && treePaths!=null))
									{
                                        ...
                                        */
										...
										final TopScoreDocCollectorManager collectorManager = new TopScoreDocCollectorManager(hitsPerPage, Integer.MAX_VALUE);  // TODO: score the results

										final ScoreDoc[] hits;
										if (fullSearch)
										{
											final TopDocs topDocs = (defaultSearchTypeButton.isSelected() ? defaultSearcher : (arabicRootsSearchTypeButton.isSelected() ? arabicRootsSearcher : arabicLuceneSearcher)).search(query, collectorManager); // Version 1.9
										...
										else
										{
											...
											final TopDocs topDocs = (defaultSearchTypeButton.isSelected() ? defaultSearcher : (arabicRootsSearchTypeButton.isSelected() ? arabicRootsSearcher : arabicLuceneSearcher)).search(finalBooleanQueryBuilder.build(), collectorManager);
											hits = topDocs.scoreDocs;
										}
										...
										{
											final StoredFields storedFields = defaultSearcher.storedFields();
											final TermVectors termVectors = defaultSearcher.getIndexReader().termVectors();
											for (int j = 0, index = 0; j < hits.length && !stopSearch; j++)
											{
												final Document doc = storedFields.document(hits[j].doc);
												final String txt = doc.get("pageContent");
												String text = "";

												final TokenStream tokenStream = TokenSources.getTokenStream("pageContent", termVectors.get(hits[j].doc), txt, language != lang.English ? arabicAnalyzer : new StandardAnalyzer(), highlighter.getMaxDocCharsToAnalyze() - 1); // Version 1.9
										...
										else
										{
											...
											{
												final StoredFields storedFields = arabicRootsSearcher.storedFields();
												final TermVectors termVectors = arabicRootsSearcher.getIndexReader().termVectors();
												for (int j = 0, index = 0; j < hits.length && !stopSearch; j++)
												{
													final Document doc = storedFields.document(hits[j].doc);
													final String txt = doc.get("pageContent");
													...
													final TokenStream tokenStream = TokenSources.getTokenStream("pageContent", termVectors.get(hits[j].doc), txt, arabicRootsAnalyzer, highlighter.getMaxDocCharsToAnalyze() - 1); // Version 1.9
													final TextFragment[] frag = highlighter.getBestTextFragments(tokenStream, txt, false, 1);
											...
											{
												final StoredFields storedFields = arabicLuceneSearcher.storedFields();
												final TermVectors termVectors = arabicLuceneSearcher.getIndexReader().termVectors();
												for (int j = 0, index = 0; j < hits.length && !stopSearch; j++)
												{
													final Document doc = storedFields.document(hits[j].doc);
													final String txt = doc.get("pageContent");
													...
													final TokenStream tokenStream = TokenSources.getTokenStream("pageContent", termVectors.get(hits[j].doc), txt, arabicLuceneAnalyzer, highlighter.getMaxDocCharsToAnalyze() - 1); // Version 1.9
													final TextFragment[] frag = highlighter.getBestTextFragments(tokenStream, txt, false, 1);
													for (TextFragment tf : frag)
														if ((tf != null) && (tf.getScore() > 0))
